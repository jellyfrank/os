...
class WebPdfFileTokenView(http.Controller):
    @http.route('/web/report/pdf_token', type='json', auth='user')
...
class WebPdfReports(http.Controller):
    POLLING_DELAY = 0.25
    TYPES_MAPPING = {
    ...
    @http.route('/web/report/pdf', type='http', auth='user')
    def index(self, req, pdf_file_token):
        ...
        report_id = report_srv.report(req.session._db, req.session._uid, req.session._password,action["report_name"], report_ids,report_data, context)
